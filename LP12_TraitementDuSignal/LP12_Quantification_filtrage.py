# -*- coding: utf-8 -*-
"""
Created on Wed Apr 26 18:48:30 2023
...
plt.subplots_adjust(bottom=0.25,left=0.1,right=.7)
...
l, = plt.plot(t, np.sin(2*pi*t), lw=1, color='black',label="Signal d'entrée")         # courbe Ã  tracer i en fonction de omega
lq, = plt.step(t1,quant_vect(np.sin(2*pi*t1),2048), lw=.8, color='red',where='post',label="Signal numérisé")
lfa, = plt.plot(t,np.sin(2*pi*t+np.angle(H_vect(1,1)))*np.abs(H_vect(1,1)),lw=1,color="blue",label="Filtrage analogique")
lfn, = plt.step(t1,filtrePB(quant_vect(np.sin(2*pi*t1),2048),0,2*pi,1/50,'Dérivation'),lw=.8,color="green", where='post',label="Filtrage numérique")
